-- Migration: 007_fix_database_consistency.sql
-- 描述: 修正資料庫設計一致性问题
-- 日期: 2025-01-27
-- 版本: v2.1
...
-- ============================================================================
-- 2. 新增向量嵌入支援到 places 和 accommodations 表
-- ============================================================================

-- 新增 embedding 欄位到 places 表
ALTER TABLE places ADD COLUMN IF NOT EXISTS embedding vector(1536);

-- 新增 embedding 欄位到 accommodations 表
ALTER TABLE accommodations ADD COLUMN IF NOT EXISTS embedding vector(1536);

-- 為向量搜尋建立索引（使用 HNSW 算法）
CREATE INDEX IF NOT EXISTS idx_places_embedding_hnsw 
ON places USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

CREATE INDEX IF NOT EXISTS idx_accommodations_embedding_hnsw 
ON accommodations USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- ============================================================================
-- 3. 統一時間戳記類型為 TIMESTAMPTZ
-- ============================================================================
...
-- 為 embedding 欄位建立額外的索引
CREATE INDEX IF NOT EXISTS idx_places_embedding_ivfflat 
ON places USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 100);

CREATE INDEX IF NOT EXISTS idx_accommodations_embedding_ivfflat 
ON accommodations USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 100);

-- ============================================================================
-- 6. 更新觸發器函數以支援 TIMESTAMPTZ
-- ============================================================================
